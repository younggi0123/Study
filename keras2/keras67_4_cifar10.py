...
# & Autokeras로 돌린 것도 비교 !
# 위 5가지 case 결과와, 지금까지 본인이 cifar10을 돌려서 최고치를 비교
...
# Dense layer output의 정식 명칭은?? units
# parameters :  pool_size=(2,2)   pool_size=2(정수 가능)  pooling에 사용할 filter의 크기를 정함(단순한 정수, 또는 튜플형태 (N,N))# parameters :  pool_size=(2,2)   pool_size=2(정수 가능)  pooling에 사용할 filter의 크기를 정함(단순한 정수, 또는 튜플형태 (N,N))
