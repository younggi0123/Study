...
model.fit(x, y, epochs=100000, batch_size=1)   # epochs로 n번의 선을 그어 훈련시키겠다. batch_size는 x와y를 한번에 다 집어 넣는게 아닌, 한게씩 집어 넣는게 훈련 잘 됨(1개씩 넣겠단 말) #batch가 작을수록 훈련이 잘 되지만 표본이 너무 많으면 힘들 수 있다(속도)
...
print('loss : ', loss)
result = model.predict([6])         # 6은 새로운 x값
