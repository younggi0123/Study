...
# np.save('./_save_npy/keras48_2_validation_x.npy', arr=validation_generator[0][0])
# np.save('./_save_npy/keras48_2_validation_y.npy', arr=validation_generator[0][1])
...
x_validation = np.load('./_save_npy/keras48_2_validation_x.npy')
y_validation = np.load('./_save_npy/keras48_2_validation_y.npy')
...
# steps_per_epoch= spe
import os
path = "./_save/horse_or_human_npy_1.h5"
print(os.getcwd())
if os.path.exists(path):
    ...
    model.save("./_save/horse_or_human_npy_1.h5")
...
print("Between horse_or_human data Accuracy : ",str(np.round(acc ,2)*100)+ "%")# 여기서 accuracy는 이 밑의 샘플데이터에 대한 관측치가 아니고 모델 내에서 학습하고 평가한 정확도임
...
# print(classes)          # [[0.9280887]]

y_predict = model.predict(images, batch_size=40)
print(y_predict)        # 0.9280887
...
# between horse_or_human data Accuracy :  66.0%
