...
# groupby 쓰고, count() 쓰기 
# ㄴ 데이터 셋의 그룹바이 (기준은 quality임)
# plt.bar로 그려라 ! quality

# 나
import matplotlib.pyplot as plt
# gb_datasets = datasets.groupby('quality').count()
# gb_datasets.plot(kind='bar',rot=0)
# plt.show()
# # x = np.arange(30)
# # # plt.figure( figsize=(300, 300) )
# # plt.bar(x, gb_datasets )
# # plt.show()
...
# 선생님
# quality안의 라벨별로 묶으려면?퀄리티.count()
count_data = datasets.groupby('quality')['quality'].count()
print(count_data)

plt.bar(count_data.index, count_data)  #(가로 세로 필요)=> index 넣어주기
plt.show()


# target값이 6에 몰려있는걸 알 수 있다.
# 하여, 결과값이 잘 해야 7초반이겠지??
######################################################################
